...
export const Personal = () => {
        ...
        const [persons, setPersonData] = useState([]);//store household data
        ...
        useEffect(() => {
            fetchPersonalData(ward,search,rows,offset,ordering,id,PARENT_KEY);
          }, []);
        
        const fetchPersonalData = async (ward,search,rows,offset,ordering,id,PARENT_KEY) => {
            try {
              const data = await apiService.getPersonal(ward,search,rows,offset,ordering,id,PARENT_KEY); // You may need to adjust the API request to include pagination parameters
        ...
        const actionBodyTemplate = (rowData) => {
            return (
                ...
                <div className="actions">
                    ...
                    <Link to={`/personaldetail/${findIndexById(rowData.id)}`}>
